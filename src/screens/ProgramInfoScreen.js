...
const COURSES_BY_MAJOR = {
  cs: [
    ...
    {
      label: "CS 3503 - Computer Organization & Architecture",
      value: "CS3503",
    },
  ...
  it: [
    ...
    { label: "POLS 1101 - American Government", value: "POLS1101" },
    {
      label: "IT 3703 - Intro to Data Analytics & Technology",
      value: "IT3703",
    },
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={styles.backText}>Back to School Info</Text>
      </TouchableOpacity>

      <Text style={styles.title}>Program Info</Text>
      <RegistrationStepsHeader activeIndex={3} />

      <View>
        <Text style={styles.headerText}>
          Select courses you've already completed
        </Text>
        <TextInput
          style={styles.search}
      ...
      {completedCourses.length === 0 ? null : (
        <View
          style={{
        ...
        >
          <Text style={styles.selectedHeaderText}>
            Selected ({completedCourses.length})
          </Text>
            ...
            {completedCourses.map((val) => {
              ...
              return (
                <TouchableOpacity key={val} onPress={() => toggleCourse(val)}>
                  <Text style={styles.selectedCourse}>{course?.value}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
      ...
      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10 }}>
        {filtered.map((course) => (
          ...
          >
            <Text style={styles.courseText}>{course.value}</Text>
          </TouchableOpacity>
        ))}
      </View>
...
const styles = StyleSheet.create({
  ...
  selectedHeaderText: { color: "#4F46E4", fontWeight: 700, marginBottom: 10 },
});
